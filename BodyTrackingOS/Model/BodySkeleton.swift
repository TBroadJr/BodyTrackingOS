...
class BodySkeleton: Entity {
    ...
    required init(for bodyAnchor: ARBodyAnchor) {
        ...
        for jointName in ARSkeletonDefinition.defaultBody3D.jointNames {
            ...
            // set size and color based on specific jointName
            // Green joints are actively tracked by ARKit. Yellow joints are not tracked, they follow the motion of the closest green parent
            switch jointName {
            case "neck_1_joint", "neck_2_joint", "neck_3_joint", "neck_4_joint", "head_joint", "left_shoulder_joint_1", "right_shoulder_joint_1":
                jointRadius *= 0.5
            ...
            // create an entity for the joint, adds joint to dictionary and add it to the parent entity(i.e bodySkeleton)
        ...
        for bone in Bones.allCases {
            ...
            // create an entity for the bone, adds bone to dictionary, and add it to the parent entity(i.e bodySkeleton
    ...
    private func createSkeletonBone(bone: Bones, bodyAnchor: ARBodyAnchor) -> SkeletonBone? {
        ...
        let jointFromEntityPosition = jointFromEntityOffsetFromRoot + rootPosition // relative to world reference
        
        let jointToEntityOffsetFromRoot = simd_make_float3(toJointEntityTransform.columns.3) // relative to root(i.e hipjoint)
        let jointToEntityPosition = jointToEntityOffsetFromRoot + rootPosition // relative to world reference
